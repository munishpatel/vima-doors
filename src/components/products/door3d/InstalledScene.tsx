 ...
 *
 * It is the exact `DoorModel` the blueprint half explodes, so the banner never
 * has to promise that the drawing and the installed door match — they are the
 * same mesh. The room around it is kept deliberately quiet: limewashed wall,
 * classical wall mouldings, a polished ivory vitrified floor, and a console with a lamp so it reads as somebody's home rather
 * than a showroom.
 */
...
export default function InstalledScene({ paused, reducedMotion }: InstalledSceneProps) {
  return (
    <Canvas
      shadows
    ...
    >
      ...
      <Floor />
      <Console />
      <DoorModel activeId={null} hoverId={null} />
    </Canvas>
  );
}
